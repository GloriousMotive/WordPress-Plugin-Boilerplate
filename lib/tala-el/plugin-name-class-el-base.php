<?php
if(!class_exists("Plugin_Name_EL_Base")) {
	class Plugin_Name_EL_Base {
    	public $key = PLUGIN_NAME_EL_KEY;
    	private $product_id = PLUGIN_NAME_EL_PID;
    	private $product_base = "plugin_name";
    	private $server_host = "https://gloriousmotive.com/wp-json/tala/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_plugin_name_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=plugin_name_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","plugin_name").'</h1>');
